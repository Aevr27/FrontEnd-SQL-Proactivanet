...
/* Escape de HTML compartido (assets/js/escape.js): lo que viene del JSON
   -nombre y descripcion de cada aplicacion, propietario, PO, Service Owner,
   niveles de madurez- se pega con innerHTML y tiene que entrar como TEXTO. */
...
function renderObserv(){
  ...
  document.getElementById('kpisOb').innerHTML=cards.map(c=>
    `<div class="kpi"><div class="lbl">${c.l}</div><div class="val" style="font-size:${c.v.length>10?'16px':'25px'}">${esc(c.v)}</div><div class="foot">${c.f}</div></div>`).join('');
  ...
  document.getElementById('nivelesGrid').innerHTML=niveles.map(n=>{
    ...
    return `<div class="nivelBox" data-nivel="${Escape.attr(n.nivel)}"
      ...
      title="${Escape.attr(n.desc)}">
      <div style="font-size:26px">${esc(n.icono)}</div>
      <div style="font-size:19px;font-weight:800;margin:2px 0">${q}</div>
      <div style="font-size:10px;color:var(--muted);font-weight:600;line-height:1.2">${esc(n.nivel)}</div></div>`;
  }).join('');
  ...
  document.getElementById('bodyApps').innerHTML = total? apps.map(a=>
    `<tr><td><b>${esc(a.nombre)}</b></td><td style="max-width:240px;font-size:11px">${esc(a.descripcion||'—')}</td>
     <td>${(a.bia||'').toUpperCase()==='SI'?'<span class="chip" style="background:#fdf4f2;color:#982a18">Crítico</span>':esc(a.bia||'—')}</td>
